...
document.querySelector("#confirm-deposit").addEventListener("click", () => {
  ...
  if (activeDepositButton) {
    ...
    setTimeout(() => { activeDepositButton.textContent = originalText; }, 2200);
  } else {
    document.querySelector("#new-vault-button").textContent = "Vault started ✓";
    setTimeout(() => { document.querySelector("#new-vault-button").innerHTML = "<span>+</span> New vault"; }, 2200);
  }
});
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && backdrop.classList.contains("visible")) closeModal();
});
document.querySelectorAll(".feature-link").forEach((button) => {
  button.addEventListener("click", () => {
    button.textContent = "Coming next ✓";
    setTimeout(() => { button.innerHTML = `${button.dataset.feature === "shift-to-save" ? "Set a rule" : button.dataset.feature === "safe-to-spend" ? "View your plan" : button.dataset.feature === "portable" ? "Connect a workplace" : "See how it works"} <span>→</span>`; }, 1800);
  });
  ...
  actionBackdrop.addEventListener("click", (event) => {
    if (event.target === actionBackdrop) actionBackdrop.classList.remove("visible");
  });
